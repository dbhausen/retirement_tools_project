...
abstract class AbstractPerson {
	...
	abstract getProbabilityOfDeathByAge(targetAge: number): number
	// Return the probability that this individual will die by target age.

	// eslint-disable-next-line no-unused-vars
	abstract getProbabilityOfDeathAtAge(targetAge: number): number
	// Return the probability that this individual will die by target age.

	// eslint-disable-next-line no-unused-vars
	abstract getProbabilityOfLivingToAge(targetAge: number): number
...
class Person extends AbstractPerson {
	...
	getLifeExpectancy = (): number => {
		// Returns the expected age to be reached by person of this age and sex.
		if (this.sex === MALE) {
			return LIFE[this.age][MALE_LIFE_EXPECTANCY] + this.age
		}
		return LIFE[this.age][FEMALE_LIFE_EXPECTANCY] + this.age
	}
	...
	getProbabilityOfLivingToAge = (targetAge: number): number =>
		// Return the probability that this individual will live to age.//
		1 - this.getProbabilityOfDeathByAge(targetAge)
}
...
abstract class AbstractCouple {
	...
	// eslint-disable-next-line no-unused-vars
	abstract getProbabilityOfNeitherReachingTargetAge(targetAge: number): number
	// Returns the probability that neither one of this couple will reach target age.//

	// eslint-disable-next-line no-unused-vars
	abstract getProbabilityOfBothReachingTargetAge(targetAge: number): number
	// Returns the probability that both of this couple will reach target age.

	abstract getProbabilityOfExactlyOneReachingTargetAge(
	// eslint-disable-next-line no-unused-vars
		targetAge: number
	): number
...
class Couple extends AbstractCouple {
	...
	getAgeOfOldest(): number {
		return this.person1.age > this.person2.age
			? this.person1.age
			: this.person2.age
	}

	getAgeOfYoungest(): number {
		return this.person1.age < this.person2.age
			? this.person1.age
			: this.person2.age
	}
	...
	getProbabilityOfExactlyOneReachingTargetAge = (targetAge: number): number =>
		...
		1 -
		this.getProbabilityOfBothReachingTargetAge(targetAge) -
